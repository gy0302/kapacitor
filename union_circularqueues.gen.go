...
type timeMessageCircularQueue struct {
	data    []timeMessage
	head    int
	tail    int
	l       int
...
// newTimeMessageconstructs a Circular Queue
// with a given buffer buf.  It is ok for buf to be nil.
func newTimeMessageCircularQueue(buf ...timeMessage) *timeMessageCircularQueue {
	return &timeMessageCircularQueue{
		data:    buf,
		head:    0,
		tail:    len(buf),
		l:       len(buf),
...
func (q *timeMessageCircularQueue) Enqueue(v ...timeMessage) bool {
...
func (q *timeMessageCircularQueue) Next() bool {

	// zero the old q.head to prevent leaking of sub fields.
	var fill timeMessage
	if q.oldHead >= 0 {
...
func (q *timeMessageCircularQueue) Val() timeMessage {
	return q.data[q.head-1]
}

// Peek looks at the queue without dequeueing.
func (q *timeMessageCircularQueue) Peek(i int) (timeMessage, bool) {
...
func (q *timeMessageCircularQueue) Len() int {
	return q.l
}
